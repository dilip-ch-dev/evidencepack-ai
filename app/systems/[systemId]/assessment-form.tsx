...
export function AssessmentForm({ systemId }: Props) {
  ...
  return (
    <form action={formAction} className="grid gap-3">
      ...
      <SubmitButton pendingLabel="Generating grounded assessment…" className="inline-flex items-center justify-center rounded-xl bg-sky-600 px-4 py-3 text-sm font-medium text-white shadow-sm hover:bg-sky-700">
        Generate assessment
      </SubmitButton>
      {state.status === "error" && (
